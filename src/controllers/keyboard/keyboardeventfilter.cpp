    ...
    if (e->type() == QEvent::FocusOut) {
        ...
        if (!ks.isEmpty()) {
            ...
            for (auto it = m_keySequenceToControlHash.constFind(ksv);
                ...
                if (configKey.group != "[KeyboardShortcuts]") {
                    ...
                    if (control) {
                        //qDebug() << configKey << "MidiOpCode::NOTE_ON" << 1;
                        ...
                        control->setValueFromMidi(MidiOpCode::NOTE_ON, 1);
                        result = true;
                    } else {
        ...
        for (int i = m_qActiveKeyList.size() - 1; i >= 0; i--) {
            ...
            if (keyDownInfo.keyId == keyId ||
                ...
                if (!autoRepeat) {
                    //qDebug() << pControl->getKey() << "MidiOpCode::NOTE_OFF" << 0;
                    pControl->setValueFromMidi(MidiOpCode::NOTE_OFF, 0);
