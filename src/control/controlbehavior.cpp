...
void ControlPushButtonBehavior::setValueFromMidi(
    ...
    // Some controller like the RMX2 are sending always MidiOpCode::NOTE_ON
    // with a changed dParam 127 for pressed an 0 for released.
    // Other controller like the VMS4 are using MidiOpCode::NOTE_ON
    // And MidiOpCode::NOTE_OFF and a velocity value like a piano keyboard
    bool pressed = true;
    if (o == MidiOpCode::NOTE_OFF || dParam == 0) {
        // MidiOpCode::NOTE_ON + 0 should be interpreted a released according to
        // http://de.wikipedia.org/wiki/Musical_Instrument_Digital_Interface
        // looking for MidiOpCode::NOTE_ON doesn't seem to work...
        pressed = false;
    }
