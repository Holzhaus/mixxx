...
void DlgPreferences::slotButtonPressed(QAbstractButton* pButton) {
    ...
    switch (role) {
        ...
        case QDialogButtonBox::HelpRole:
            if (pCurrentPage) {
                QString helpDocument = pCurrentPage->helpDocument();
                DEBUG_ASSERT(!helpDocument.isEmpty());
                emit showManual(helpDocument);
            }
            break;
        default:
            break;
...
void DlgPreferences::addPageWidget(PreferencesPage page) {
    ...
    connect(page.pDlg, &DlgPreferencePage::showManual, this, &DlgPreferences::showManual);
