...
bool CmdlineArgs::parse(const QStringList& arguments) {
    ...
    QCommandLineOption qmlPath(QStringList() << "qml",
            QCoreApplication::translate("main",
                    "Load GUI from a QML file instead of legacy skin system."),
            QStringLiteral("qml"));
    parser.addOption(qmlPath);
    ...
    if (parser.isSet(qmlPath)) {
        m_qmlPath = parser.value(qmlPath);
    }
